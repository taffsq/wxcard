...
class Dao{
    ...
    public function getGoodsLike( $val ){
        $db = $this->getDb();
        $sql = "SELECT * FROM goods_table where goods_name like ?";
        ...
        if( (boolean)$stmt = $db->prepare( $sql ) ){
            ...
            while( $stmt->fetch() )
                $arr[] = array( 'id'=>$_id, 'name'=>$_name, 'img'=>$_img, 'desc'=>$_desc, "b_pri"=>$_bpri, "pri"=>$_pri, 'tel'=>$_tel, 'aid'=>$_aid,'cid'=>$_cid,'qrcode'=>$_qr,'at'=>$_atime );
